...
-- lam (app (var 0) (var 1)) : Trm 2 (var 0 is bound while var 1 is free)
  ...
  liftFin-fs' : ∀ {n} → liftFin {n} fs ∘ fs == fs ∘ fs
  liftFin-fs' {n} = FunExt (λ _ → =rf)
    
  liftFin-cmp : {n m k : Nat}(f : Fin n → Fin m)(g : Fin m → Fin k)
                   → liftFin g ∘ liftFin f == liftFin (g ∘ f)
  liftFin-cmp f g = FunExt (liftFin-cmp-ptw f g)

  liftFin-cmp⁻¹ : {n m k : Nat}(f : Fin n → Fin m)(g : Fin m → Fin k)
                   → liftFin (g ∘ f) == liftFin g ∘ liftFin f
  liftFin-cmp⁻¹ f g = liftFin-cmp f g ⁻¹
  ...
  wlift-cmp-ptw : {n m k : Nat}(f : Fin n → Fin m)(g : Fin m → Trm k)
                → (i : Fin (suc n)) → wlift g (liftFin f i) == wlift (g ∘ f) i
  wlift-cmp-ptw f g fz = =rf
  wlift-cmp-ptw f g (fs i) = =rf

  wlift-cmp : {n m k : Nat}(f : Fin n → Fin m)(g : Fin m → Trm k)
                → wlift g ∘ liftFin f == wlift (g ∘ f)
  wlift-cmp f g = FunExt (wlift-cmp-ptw f g)

  wlift-cmp⁻¹ : {n m k : Nat}(f : Fin n → Fin m)(g : Fin m → Trm k)
                → wlift (g ∘ f) == wlift g ∘ liftFin f
  wlift-cmp⁻¹ f g = wlift-cmp f g ⁻¹
  ...
  rename-act (lam M) f g =
    =ap lam (=proof
      ...
      rename M (liftFin g ∘ liftFin f)           ==[ =ap (rename M) (liftFin-cmp f g) ]∎
  ...
  rename-sq (lam M) {f} {f'} {g} {g'} pf =
    =ap lam (=proof
      rename M (liftFin (g' ∘ f))            ==[ =ap (rename M) (liftFin-cmp⁻¹ f g') ] /
  ...
  subst-all-rename (lam M) f g =
    =ap lam (=proof
      ...
      subst-all M (wlift g ∘ liftFin f)            ==[ =ap (subst-all M) (wlift-cmp f g) ]∎
  ...
  wlift-var-ptw :  {n : Nat} → ∀ i → wlift (var {n}) i == var i
  wlift-var-ptw fz = =rf
  wlift-var-ptw (fs i) = =rf

  wlift-var :  {n : Nat} → wlift (var {n}) == var {suc n}
  wlift-var {n} = FunExt (wlift-var-ptw {n})
  ...
  wlift-glsect-lift-ptw : {n : Nat}(N : Trm n) → ∀ i
    → wlift (glsect N) (liftFin fs i) == var i
  wlift-glsect-lift-ptw N fz = =rf
  wlift-glsect-lift-ptw N (fs i) = =rf

  wlift-glsect-lift : {n : Nat}(N : Trm n) → wlift (glsect N) ∘ liftFin fs == var
  wlift-glsect-lift N = FunExt (wlift-glsect-lift-ptw N) 
  ...
  subst-all-var (var x) =
    =rf
  subst-all-var (lam M) =
    =ap lam (=ap (subst-all M) wlift-var • subst-all-var M)
  ...
  subst-all-glsect-lift M N = =ap (subst-all M) (wlift-glsect-lift N) • subst-all-var M
  ...
  subst-ass-wlift-ptw : {n m k : Nat}(f : Fin n → Trm m)(g : Fin m → Trm k)(i : Fin (suc n))
                          → (subst-all (wlift f i) (wlift g)) == (wlift (λ i → subst-all (f i) g) i)
  subst-ass-wlift-ptw f g fz =
    =rf {a = var fz}
  subst-ass-wlift-ptw f g (fs i) =
    subst-all-ext (f i) g

  -- need equality between these families of terms to prove distribuitivity of substitution
  subst-ass-wlift : {n m k : Nat}(f : Fin n → Trm m)(g : Fin m → Trm k)
                      → (λ i → subst-all (wlift f i) (wlift g)) == (wlift (λ i → subst-all (f i) g))
  subst-ass-wlift f g = FunExt (subst-ass-wlift-ptw f g) 
  ...
  subst-all-dist (lam M) f g =
    =ap lam (=proof
      ...
      subst-all M (λ i → subst-all (wlift f i) (wlift g))  ==[ =ap (subst-all M) (subst-ass-wlift f g) ]∎
